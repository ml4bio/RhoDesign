...
class GVPTransformerEncoder(nn.Module):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.embed_gvp_ss_input = nn.Linear(300, embed_dim)
        ...
        gvp_out_dim = gvp_args.node_hidden_dim_scalar + (3 *
                gvp_args.node_hidden_dim_vector) + 300 # 100 for the ss_ct_map features
    ...
    def forward_embedding(self, coords, adjunct_coords, ss_ct_map,padding_mask, confidence):
        """
        Args:
        ...
        # print(gvp_out_scalars.shape,gvp_out_vectors.shape)
        gvp_out_features = torch.cat([
            gvp_out_scalars,
            rotate(gvp_out_vectors, R.transpose(-2, -1)).flatten(-2, -1),
        ], dim=-1)
        try:
            out = torch.cat([gvp_out_features,ss_ct_map],dim=-1)
        except:
            raise ValueError(f"GVP output! gvp_out_features.shape: {gvp_out_features.shape} and {ss_ct_map.shape}")
        
        # print(gvp_out_features.shape,ss_ct_map.shape)
        
        components["gvp_out"] = self.embed_gvp_output(torch.cat([gvp_out_features,ss_ct_map],dim=-1)) #concat gvp_output + ss_features
        ...
        ss_ct_map = ss_ct_map.float()
        components["gvp_ss_input"] = self.embed_gvp_ss_input(ss_ct_map)
    ...
    def forward(
        ...
        """
        x, encoder_embedding = self.forward_embedding(coords,adjunct_coords,ss_ct_map,encoder_padding_mask, confidence)
